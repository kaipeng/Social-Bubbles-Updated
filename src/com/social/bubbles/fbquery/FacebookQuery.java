...
public class FacebookQuery{
	//Fetching parameters
		...
		public static final int RECENT_CHECKINS_RANK_OFFSET= 24; //hours
		public static final int FRIENDS_EVENTS_RANK_OFFSET= 3; //hours
		...
		//Pictures
		private final String PICTURE_SIZE = "large";
	...
	public LinkedList<FacebookObject> getFacebookRecentCheckins(){
		...
		LinkedList<FacebookUser> recentFbUsers = new LinkedList<FacebookUser>();
		FacebookCheckinData data= getRecentFriendCheckins();
		if(data!=null)
			recentFbUsers = convertCheckinsToUsers(getRecentFriendCheckins().getData());
		...
		for(FacebookUser aUser : recentFbUsers){			//all recentFbUsers & set time to last checkin - NEARBY_CHECKINS_RANK_OFFSET hours
			...
			aUser.setLastActivity(new Date((actual.getTime())));
			...
			if(aUser.getFacebookEventData() != null){
				...
				if(friendsEvents!=null){
					...
					for(FacebookEvent anEvent : friendsEvents){		//all friends' events & set time to start time - OFFSET
						...
						anEvent.setLastActivity(new Date((actual.getTime())));
...
//				        params.putString("type", PICTURE_SIZE);
						...
						FacebookObject picData=null;
						try{
							picData= new Gson().fromJson(result, FacebookObject.class);
						}catch(Exception e){};
	...
	private FacebookUser getUser(String id){
    	...
    	FacebookUser a = null;
    	try{
    		a= new Gson().fromJson(result,FacebookUser.class);
    	}catch(Exception e){};
		return a;
	}
	...
	private LinkedList<FacebookPlace> getPlacesAroundUser(){
		...
		FacebookPlaceData fPlaceData = null;
        try   {   
         fPlaceData =  new Gson().fromJson(result,FacebookPlaceData.class);
	...
	private FacebookCheckinData getRecentFriendCheckins(){
    	...
    	try{
    	FacebookCheckinData data = new Gson().fromJson(result,FacebookCheckinData.class);
        return data;
	...
	private FacebookEventData getEvents(){
		FacebookEventData eventData = null;
		 Bundle params = new Bundle();
